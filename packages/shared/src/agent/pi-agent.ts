...
export class PiAgent extends BaseAgent {
  ...
  private async spawnSubprocess(): Promise<void> {
    ...
    if (!nodePath) {
      throw new Error('Bundled runtime path not configured. Cannot spawn Pi subprocess.');
    }
    ...
    try {
      ...
      if (images.length > 0) {
        ...
        if (runtime.customEndpoint) {
          ...
          const modelCapability = (runtime.customModels ?? []).find(m =>
            typeof m !== 'string' &&
            (m.id === bareModel || m.id === this._model || `pi/${m.id}` === this._model),
          );
          const imageEnabled = (
            modelCapability && typeof modelCapability !== 'string' && typeof modelCapability.supportsImages === 'boolean'
          )
            ? modelCapability.supportsImages
            : runtime.customEndpoint.supportsImages ?? false;
